...
Railway uchun environment variables:
...
# ═══════════════════════════════════════════════
# ⚙️  SOZLAMALAR  (Railway → Variables ga qo'ying)
# ═══════════════════════════════════════════════
ADMIN_ID = int(os.getenv("ADMIN_ID", "0"))   # @userinfobot dan oling
...
def get_plans(user_id: int,
    ...
    with db() as c:
        ...
        p = [user_id]
        if date:
            q += " AND plan_date=?"; p.append(date)
        if ptype:
            q += " AND ptype=?"; p.append(ptype)
...
def update_plan(plan_id: int, upd: PlanUpdate, user_id: int):
    with db() as c:
        exists = c.execute(
            "SELECT id FROM plans WHERE id=? AND user_id=?", (plan_id, user_id)
        ).fetchone()
        if not exists:
...
def admin_plans(admin_id: int,
    ...
    with db() as c:
        ...
        if plan_date:
            q += " AND p.plan_date=?"; params.append(plan_date)
